...
class Preprocessor(object):
    ...
    def process_actions(self, ex, traj):
        ...
        for a in ex['plan']['high_pddl']:
            if 'parameter' in a['discrete_action']:
                low_len = len(a['discrete_action']['parameter'])
                full_args = []
                for l_idx in range(low_len):
                    full_args += a['discrete_action']['parameter'][l_idx]["args"]

                a['discrete_action']['args'] = [
                    w.strip().lower() for w in full_args]
            else:
                a['discrete_action']['args'] = [
