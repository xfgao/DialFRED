...
class EvalMaster(object):
    def __init__(self, args, model_path):
        ...
        print(data_name, self.args.split, model_args, args.data['ann_type'])
        ...
        while True:
            if log_queue.qsize() > 0:
                ...
                if not test_split:
                    ...
                    gcrs = [
                        log['goal_condition_success'] for log in results[eval_epoch][eval_type].values()]
                    print(colored(
                        '{:4d}/{} trials are done (current SR = {:.1f}), (current GCR = {:.1f}), ETA = {}, elapsed = {}'.format(
                            self.num_trials_done,
                            self.num_trials,
                            100 * sum(successes) / len(successes),
                            100 * sum(gcrs) / len(gcrs),
